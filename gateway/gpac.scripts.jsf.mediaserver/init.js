...
// Initialize global statistics object from services_defs config
function initGatewayStats(services_defs) {
    const stats = {};
    for (const svc of services_defs) {
        stats[svc.id] = {
            ...
            repair: svc.repair ?? false,
            timeshift: svc.timeshift ?? 0,
            servedRequests: 0,
            servedFrom: {
                http: 0,
                mcast: 0,
...
function getServiceStatus(serviceStats, serviceObject) {
    const status = Object.assign({}, serviceStats, {
        servedFrom: Object.assign({}, serviceStats.servedFrom),
        cache: { mabr: 0, http: 0 },
        cacheSize: 0
    });

    if (serviceObject !== null && serviceObject !== undefined) {
        status.mcastOrigin     = serviceObject.mabr ?? null;
        status.mabrLoaded      = serviceObject.mabr_loaded ?? false;
        status.mabrFailure     = serviceObject.mabr_failure ?? false;
        status.nbMabrActive    = serviceObject.nb_mabr_active ?? 0;
        status.dynMabr         = serviceObject.dyn_mabr ?? serviceStats.dynMabr;
        status.repair          = serviceObject.repair ?? serviceStats.repair;

        const memCache = serviceObject.mem_cache;
        if (Array.isArray(memCache)) {
            status.cacheSize   = memCache.length;
            status.cache.http  = memCache.filter(e => e.cache_type === 0).length;
            status.cache.mabr  = memCache.filter(e => e.cache_type === 2).length;
        }
    }
...
// Build the full gateway status object across all services
function gatewayStatus(){
    const globalStatus = {};
	for (const [serviceId, serviceStats] of Object.entries(gatewayStats)) {
        const serviceObject = all_services
            ? (all_services.find(s => s.id === serviceId) ?? null)
            : null;
		globalStatus[serviceId] = getServiceStatus(serviceStats, serviceObject);

    }
    return {
		status: globalStatus //,
		// config: services_defs
	};
}

globalThis.gatewayStatus = gatewayStatus;
...
httpout.on_request = (req) =>
{
	...
	req.flush_request = function() {
		...
		if (this.cache_file.cache_type==CACHE_TYPE_MABR) {
			trackRequestServedFromMABR(this.service.id);
			...
			if (val == 'off-edge') trackOffEdge(this.service.id);
			this.headers_out.push( { "name" : 'X-From-MABR', "value": val} );
		}
		if(this.cache_file.cache_type==CACHE_TYPE_HTTP){
			trackRequestServedFromHTTP(this.service.id);
		}
...
filter.initialize = function() {
	...
	if (!filter.scfg || !filter.scfg.length) {
		...
		try {
			...
			gatewayStats = initGatewayStats(services_defs);
